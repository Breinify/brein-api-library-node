...
Breinify.prototype = {
    ...
    setConfig: function (config) {
        ...
        if (isUndefinedorNull(config) || isEmpty(config)) {
            throw new Error("Please configure Breinify...");
        } else { //noinspection JSUnresolvedFunction
            if (isUndefinedorNull(config.apiKey) || isEmpty(config.apiKey)) {
    ...
    activity: function (user, type, description, tags, category, sign, callback) {
        ...
        /* istanbul ignore else  */
        if (isEmpty(user) || isUndefinedorNull(user)) {
            if (config.verbose) {
            ...
            if (isEmpty(type) || isUndefinedorNull(type)) {
    ...
    _generateSignature: function (secret, payload, timestamp) {
        ...
        if (typeof secret !== 'string' ||  isUndefinedorNull(secret) || isEmpty(secret)) {
            if (config.verbose) {
        ...
        } else if (typeof payload !== 'string' || isUndefinedorNull(secret) || isEmpty(payload)) {
            if (config.verbose) {
            ...
            var message = payload + Math.floor(timestamp).toString() + payloadArr.length.toString();
